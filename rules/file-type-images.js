 ...
 */
async function fileTypeImages(fs, options) {
  const files = await fs.findAll(options.type)

  const targets = files.map(file => {
    const message = 'Images file type exists'
    return { passed: false, path: file, message }
  })

  if (targets.length === 0) {
    const message = "Images file type doesn't exist"
